...
def create_unstructured_prompt(
    node_labels: Optional[List[str]] = None, rel_types: Optional[List[str]] = None, additional_context: Optional[str] = None  # New parameter
) -> ChatPromptTemplate:
    node_labels_str = str(node_labels) if node_labels else ""
    rel_types_str = str(rel_types) if rel_types else ""
    
    # Integrating the additional context into the base string parts
    base_string_parts = [
        ...
        "crucial.",
        additional_context if additional_context else "",  # Add additional context here
    ...
    human_string_parts = [
        ...
        additional_context if additional_context else "",  # Add additional context here as well
...
def create_default_prompt_with_context(
    self, 
    ...
    node_labels_str = ', '.join(allowed_nodes) if allowed_nodes else ""
    rel_types_str = ', '.join(allowed_relationships) if allowed_relationships else ""
    ...
    base_string_parts = [
        ...
        '"head_type", "relation", "tail", and "tail_type". The "head" '
        "key must contain the text of the extracted entity with one of the types "
        f"from the following list: [{node_labels_str}].",
        f'The "relation" key must contain the type of relation between the "head" '
        f'and the "tail", which must be one of the following: [{rel_types_str}].',
        f'The "tail" key must represent the text of an extracted entity which is '
        f'the tail of the relation, and the "tail_type" key must contain the type '
        f"of the tail entity from the list [{node_labels_str}].",
        "Attempt to extract as many entities and relations as you can. Maintain "
        "Entity Consistency: When extracting entities, it's vital to ensure "
        'consistency. If an entity, such as "John Doe", is mentioned multiple '
        "times in the text but is referred to by different names or pronouns "
        ...
        "understandable, so maintaining consistency in entity references is "
        "crucial.",
        # Integrating the additional context here
    ...
    human_string_parts = [
        ...
        context if context else "",  # Add additional context here as well
        "Below are a number of examples of text and their extracted "
        "entities and relationships."
        "{examples}\n"
        "For the following text, extract entities and relations as "
...
class LLMGraphTransformer:
    ...
    def __init__(
        ...
        try:
            ...
            self._function_call = False #if LLM does not support structured output, 
        if not self._function_call:
            if node_properties or relationship_properties: #cannot set node and relationship properties if LLM does not support structured output
                raise ValueError(
            ...
            try:
                ...
                self.json_repair = json_repair # tries to repair unstructured JSON data that might be generated by the LLM
            except ImportError:
                raise ImportError(
            ...
            prompt = prompt or create_unstructured_prompt(
                allowed_nodes, allowed_relationships, context
            )
            print(prompt)
        ...
        structured_llm = llm.with_structured_output(schema, include_raw=True) # "creates" an LLM that outputs in structured format accoring to schema
        # Integrate additional context into the default prompt
        if prompt is None:
            prompt = create_default_prompt_with_context(context)
